...
def _incus_with(mocker, *, extras, local_eth0=None):
    """A double whose ``eth0`` device actually disappears on `config_device_remove`.

    Needed since `apply_container_acl`'s teardown branch now re-reads the
    device afterwards (`_warn_if_local_eth0_survived`) — a double that kept
    reporting the old device forever would make every teardown test print a
    spurious "still has a local eth0" warning regardless of what actually
    happened.
    """
    incus = mocker.MagicMock()
    incus.config_get.return_value = json.dumps(extras) if extras else None
    devices: dict[str, dict[str, str]] = {"eth0": local_eth0} if local_eth0 else {}
    ...
    def _remove_eth0(name: str, device: str, **kwargs: object) -> None:
        del name, kwargs
        if device == "eth0":
            devices.pop("eth0", None)

    incus.config_device_remove.side_effect = _remove_eth0
    return incus
...
def test_apply_container_acl_in_loose_mode_tears_the_override_down(make_cfg, tmp_path, mocker):
    ...
    # The double actually removes the device on `config_device_remove`, so
    # the post-teardown re-check must find it gone and stay quiet.
...
def test_apply_container_acl_warns_when_teardown_leaves_a_local_eth0_behind(
    ...
    """I2: `config_device_remove(missing_ok=True)` swallows every
    `IncusError` — exhausted ETag retries, an unreachable daemon, "instance
    is busy" — not just "no such local device". If the removal silently
    failed, the container is left pinned to `incusbr0` with the strict ACL
    still enforced, while `jailbee ls` may report it loose. Re-reading the
    device afterwards and warning is the only way to surface that without
    depending on Incus's error text, which cannot be verified without a
    live daemon here.
    """
    cfg = make_cfg(tmp_path / "myrepo")
    incus = _incus_with(mocker, extras=[], local_eth0={"type": "nic"})
    incus.network_acl_exists.return_value = True
    # Override the double's normal "removal actually works" side_effect to
    # simulate a swallowed failure: the call happens, but the device stays.
    incus.config_device_remove.side_effect = None
    warn = mocker.patch("jailbee.tui.warn")
    ...
    assert "myrepo-feat" in message
    assert "eth0" in message


def test_apply_container_acl_tolerates_one_host_failing_to_resolve(make_cfg, tmp_path, mocker):
    """I1: one of two extra hosts is momentarily unresolvable. The ACL must
    still be written from the survivor, and the failed host must be named
    in a warning rather than silently dropped."""
    mocker.patch(
        "jailbee.egress.resolve_with_status",
        return_value=({"nexus.corp": ["10.0.5.7"]}, {"broken.corp": "getaddrinfo: -3"}),
    )
    warn = mocker.patch("jailbee.tui.warn")
    cfg = make_cfg(tmp_path / "myrepo")
    incus = _incus_with(mocker, extras=["nexus.corp:443", "broken.corp:443"])
    incus.network_acl_exists.return_value = True

    egress_scope.apply_container_acl(cfg, incus, "myrepo-feat", mode="strict")

    incus.network_acl_set_yaml.assert_called_once()
    _, body_written = incus.network_acl_set_yaml.call_args[0]
    assert "nexus.corp" in body_written
    assert "broken.corp" not in body_written
    assert warn.called
    assert "broken.corp" in warn.call_args[0][0]


def test_apply_container_acl_leaves_the_existing_acl_untouched_when_everything_fails(
    make_cfg, tmp_path, mocker
):
    """I1: every extra host fails to resolve this cycle. Pushing an empty
    ACL would cut off access the container already has, so the existing
    ACL is left completely untouched — mirroring
    `egress_pool._refresh_container_extras`'s phase-B guard."""
    mocker.patch(
        "jailbee.egress.resolve_with_status",
        return_value=({}, {"broken.corp": "getaddrinfo: -3"}),
    )
    mocker.patch("jailbee.tui.warn")
    cfg = make_cfg(tmp_path / "myrepo")
    incus = _incus_with(mocker, extras=["broken.corp:443"])
    incus.network_acl_exists.return_value = True
...
def test_apply_container_acl_skips_nic_work_when_nothing_has_ever_resolved(
    make_cfg, tmp_path, mocker
):
    """I1: no extra ACL has ever been created for this container, and
    nothing resolves this cycle either. There is nothing to reference on
    the NIC yet, so neither the ACL nor the NIC device is touched — the
    next refresh that resolves something creates both in one pass."""
    mocker.patch(
        "jailbee.egress.resolve_with_status",
        return_value=({}, {"broken.corp": "getaddrinfo: -3"}),
    )
