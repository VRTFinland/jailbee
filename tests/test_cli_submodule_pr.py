...
def test_adopted_pr_publishes_to_the_adopted_head_even_when_recording_it_fails(mocker, tmp_path):
    ...
    assert result.exit_code == 0, result.output
    create.assert_not_called()
    view.assert_called_once_with(tmp_path / "lib/a", "user/existing", repo="acme/lib-a")
...
@pytest.mark.parametrize("outcome", ["create", "existing", "failed_edit", "update"])
def test_submodule_outbox_pins_commands_and_consumes_an_existing_hint_once(
    mocker, tmp_path, outcome
):
    ...
    consumption = mocker.spy(pr_flow, "record_outbox_consumption")
    ...
    def run(cmd, **kwargs):
        assert kwargs["cwd"] == tmp_path / "lib/a"
        if cmd[0] == "git":
            assert cmd == ["git", "remote", "get-url", "origin"]
            return CompletedProcess(cmd, 0, "https://github.com/acme/lib-a", "")
        commands.append(cmd)
        ...
        if cmd[:3] == ["gh", "pr", "view"]:
            return CompletedProcess(
                cmd, 0, '{"number":123,"url":"https://github.com/acme/lib-a/pull/123"}', ""
            )
        assert cmd[:4] == ["gh", "pr", "edit", "123"]
        return CompletedProcess(cmd, 1 if outcome == "failed_edit" else 0, "", "edit denied")
    ...
    assert pending.call_args.kwargs["scope"].repo_root == tmp_path / "lib/a"
    assert all("--repo" in cmd for cmd in commands)
    assert all(cmd[cmd.index("--repo") + 1] == "acme/lib-a" for cmd in commands)
    if outcome != "create":
        updates.assert_called_once()
        assert updates.call_args.kwargs["outbox_hint"] == (None if outcome == "update" else source)
        edit = next(cmd for cmd in commands if cmd[:3] == ["gh", "pr", "edit"])
        assert edit[edit.index("--title") + 1] == "feat: from container"
        assert edit[edit.index("--body") + 1] == "Container body."
