...
def _create(repo=".", ref="new", labels=()):
    return {"type": "create", "repo": repo, "ref": ref, "title": "New title",
            "body": "Full [body]\nSecond line", "labels": list(labels)}


def _labels(**changes):
    return {"type": "labels", "repo": ".", "issue": 7, "add": ["feature"],
            "remove": ["bug"], "expected": {"labels": ["BUG"]}, **changes}


def _state():
    return {"type": "state", "repo": ".", "issue": 7, "state": "closed",
            "reason": "completed", "expected": {"state": "open"}}
...
def preflight(tmp_path, mocker, make_cfg):
    ...
    targets = {".": issue_outbox.RepoTarget(".", tmp_path, "acme/app"),
    ...
    labels = mocker.patch.object(issue_github, "list_labels",
                               return_value={"bug": "Bug", "feature": "Feature"})
    snapshot = issue_github.IssueSnapshot(7, "Old title", "Old body", ("Bug",), "open",
                                         "https://github.com/acme/app/issues/7", False)
    fetch = mocker.patch.object(issue_github, "get_issue", return_value=snapshot)
    mutations = [mocker.patch.object(issue_github, name) for name in
                 ("create_issue", "edit_issue", "replace_labels", "add_comment", "set_state")]
    store = JournalStore(tmp_path / "journals")

    def prepare(manifests, names=None, extras=None):
        reader.return_value = {name: json.dumps({"version": 1, "actions": actions})
                               for name, actions in manifests.items()}
        reader.return_value.update(extras or {})
        return issue_outbox.prepare_batch(cfg, incus, "test-box",
                                         list(manifests) if names is None else names,
                                         uid=1000, journal_store=store)

    yield {"prepare": prepare, "reader": reader, "fetch": fetch, "labels": labels,
           "snapshot": snapshot, "store": store, "incus": incus, "login": login,
           "identity": ContainerIdentity("test-box", "2026-09-18"), "targets": targets}
...
def test_read_issue_outbox_empty_ordering_and_sidecars(mocker):
    ...
    reader.return_value = {"z.json": "{}", "a.json": "{}", "a.progress.json": "{}",
                           "body.md": "body"}
    ...
    assert reader.call_args.args == (incus, "box", f"/home/{CONTAINER_USERNAME}/.jailbee/issue-outbox")
...
def test_outbox_read_wraps_incus_errors(mocker):
    ...
    with pytest.raises(OutboxReadError, match="box.*offline"):
...
def test_preflight_offer_action_cap(preflight, count):
    ...
    manifests = {"a.json": [_comment()] * 50, "b.json": [_comment()] * 49,
...
def test_preflight_collects_bad_manifests_and_forbidden_paths_before_github(preflight):
    ...
    with pytest.raises(IssueGateError) as caught:
        preflight["prepare"]({"bad.json": [], "forbidden.json": [_comment("untrusted")],
...
def test_preflight_caches_issues_and_canonicalizes_labels_read_only(preflight):
    batch = preflight["prepare"]({"b.json": [_create(labels=["BUG"]), _labels(), _comment()],
...
def test_preflight_rejects_pull_requests_and_caches_failed_reads(preflight):
    ...
    preflight["fetch"].side_effect = [replace(preflight["snapshot"], is_pull_request=True),
...
def test_preflight_collects_each_expected_field_difference(preflight):
    ...
    with pytest.raises(IssueGateError) as caught:
        preflight["prepare"]({"a.json": [_edit(title="New", body="New body",
                                                   expected={"title": "Wrong", "body": "Wrong"}),
                                               _labels(), _state()]})
...
def _record(preflight, actions, *, state="applied", digest=None, issue=42, repo="acme/app"):
    ...
    if state == "applied":
        store.mark_applied(key, 0, repo=repo, url="https://github.com/acme/app/issues/42", issue=issue)
...
def test_changed_digest_with_progress_is_refused(preflight, state):
    ...
    with pytest.raises(IssueGateError, match="changed.*progress"):
...
def test_preflight_rejects_changed_host_repo_for_receipt(preflight):
    ...
    with pytest.raises(IssueGateError, match="journal.*repo"):
        preflight["prepare"]({"a.json": [_create()]})


@pytest.mark.parametrize("action", [
    _edit(title="New", expected={"title": "Old title"}),
    _edit(body="New", expected={"body": "Old body"}), _labels(), _state(),
])
...
def test_pending_refs_are_manifest_local_and_do_not_fetch(preflight):
    actions = [_create(), {"type": "edit", "repo": ".", "issue_ref": "new", "body": "",
...
@pytest.mark.parametrize("field,value", [("title", "Changed"), ("body", "Changed"),
                                         ("labels", ()), ("state", "closed")])
def test_second_stale_pass_checks_each_mutated_field(preflight, field, value):
    ...
    batch = preflight["prepare"]({"a.json": [_edit(title="New", body="",
                                expected={"title": "Old title", "body": "Old body"}),
                                _labels(), _state()]})
...
def test_second_stale_pass_collects_all_differences(preflight):
    ...
    batch = preflight["prepare"]({"a.json": [_edit(title="New", body="",
                                expected={"title": "Old title", "body": "Old body"}),
                                _labels(), _state()]})
    preflight["fetch"].return_value = replace(preflight["snapshot"], title="X", body="Y",
                                             labels=(), state="closed")
    with pytest.raises(IssueStaleError) as caught:
        revalidate_batch(batch)
    assert all(f"expected.{field}" in str(caught.value) for field in ("title", "body", "labels", "state"))


def test_plan_lines_show_complete_two_repo_plan(preflight):
    from jailbee.issue_outbox import plan_lines
    batch = preflight["prepare"]({"b.json": [_edit(title="New title", body="",
                                expected={"title": "Old title", "body": "Old body"}),
                                _labels(), _state(), _comment(body="Full [comment]\nLast line")],
                                  "a.json": [_create("lib"), {"type": "comment", "repo": "lib",
                                                   "issue_ref": "new", "body": "Follow up"}]})
    assert plan_lines(batch) == [
        "Host GitHub login: alice", "Container: test-box", "Manifest: b.json",
        "Repository: . (acme/app)", "b.json action 0: edit #7 [pending]",
        "  title before:", "    Old title", "  title after:", "    New title",
        "  body before:", "    Old body", "  body after:", "    [empty]",
        "b.json action 1: labels #7 [pending]", "  remove: bug", "  add: Feature",
        "  labels after: Feature", "b.json action 2: state #7 [pending]",
        "  state: open -> closed", "  reason: completed",
        "b.json action 3: comment #7 [pending]", "  body:", "    Full [comment]", "    Last line",
        "Manifest: a.json", "Repository: lib (acme/lib)", "a.json action 0: create ref new [pending]",
        "  title:", "    New title", "  body:", "    Full [body]", "    Second line",
        "  labels: [empty]", "a.json action 1: comment ref new [pending]",
        "  depends on create ref new", "  body:", "    Follow up",
    ]


@pytest.mark.parametrize("state,display", [("applied", "applied; skip"),
                                           ("prepared", "uncertain"), ("uncertain", "uncertain")])
...
def test_uncertain_local_create_blocks_dependent_remote_assumptions(preflight):
    from jailbee.issue_outbox import revalidate_batch
    actions = [_create(), {"type": "edit", "repo": ".", "issue_ref": "new",
                           "title": "Next", "expected": {"title": "Externally resolved title"}}]
...
def test_uninitialized_submodule_uses_existing_host_root_for_all_github_reads(preflight):
    ...
    batch = preflight["prepare"]({"a.json": [
