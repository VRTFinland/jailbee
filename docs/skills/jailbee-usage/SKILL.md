...
**Whether this repo's account is shared changes what some of the commands
below actually do — establish that first.** Several repos on one host can
share a single Claude Code login (`claude_credentials` in
`~/.config/jailbee/global.yaml`, host-side and invisible from inside a
container), and on a host set up recently they usually do: a `global.yaml`
generated by `jailbee config init --global` ships `group: default`, which
puts every repo on the host in one group. Hosts whose `global.yaml` predates
that key, or that set `group: null`, keep a credential per repo. Ask the user
to run `jailbee doctor` on the host if it matters — it prints the resolved
