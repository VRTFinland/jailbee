...
| `jailbee git merge <source…> --into <target> [-b <branch>] [--plain]` | Merge one container's branch into another without a host checkout — objects travel source → host → target, no host branch, index or superproject working tree is touched (a host sub-repo can still be created, for a submodule born in the source container). `--into` is never inferred, but either end may be omitted on a TTY and is then asked for — the sources first (a checkbox that merges in *listed* order), the target second; off a TTY both must be given. Several sources are merged one at a time, in order given, stopping at the first conflict; the summary always names what landed, what stopped it, what remains, and the resume command. Conflicts are resolved inside the target with `jailbee shell <target>`. Each source that lands prints its own `── Submodules` block, read inside the target container. `--plain` transports the refs only and reports "transported", not "merged". No top-level `jailbee merge` alias exists |
