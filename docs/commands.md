...
| `jailbee claude ls [-o json] [--fields …] [-g <group>]` | List stored Claude logins and which one this repo's containers use. Shows the whole host's store, since a parked login can be activated into any credential group. `-g` reads another credential group's holder instead of this repo's |
