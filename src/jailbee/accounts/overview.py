...
def _config_homes(adapter: AccountAdapter, cfg: Config) -> tuple[dict[str, Path], list[str]]:
    ...
    for prefix, repo_root in engine.registered_repos():
        ...
        if not repo_root.is_dir():
            ...
            continue
        try:
            homes[prefix] = adapter.config_home(load_repo_config(repo_root))
        except Exception:  # ConfigError, OSError, YAML/Pydantic — all mean "unreadable"
            unreachable.append(prefix)
    homes[cfg.container_prefix] = adapter.config_home(cfg)
...
def _existing_group_dirs(adapter: AccountAdapter) -> list[str]:
    ...
    root = engine.group_dir(adapter.name, "x").parent
    try:
...
def build(adapter: AccountAdapter, cfg: Config, gcfg: GlobalConfig, incus: Incus) -> Overview:
    ...
    homes, unreachable = _config_homes(adapter, cfg)
    ...
    triples = groups.container_groups(gcfg, raw, prefixes)
    by_prefix = groups.groups_by_prefix_from(gcfg, raw, prefixes)

    mine = engine.holder_dir(adapter, cfg)
    parked = engine.parked_slots(adapter)
    taken = {s.name for s in parked}
    rows: list[Row] = []

    group_names = sorted(
        {*_existing_group_dirs(adapter)}
    ...
    for group in group_names:
        ...
        rows.append(
            Row(
                agent=adapter.name,
                slot=_named(engine.live_slot_at(adapter, holder, identity), taken),
                group=group,
                prefix=None,
                holder=holder,
    ...
    rows.extend(
        Row(agent=adapter.name, slot=s, group=None, prefix=None, holder=None) for s in parked
    )
    return Overview(
...
def _named(slot: Slot | None, taken: set[str]) -> Slot | None:
    ...
    from jailbee.accounts.models import DISAMBIGUATOR
